...
<td>
                ...
                <select name="uid" id="uid" class="login-form2" style="height:30px; width:200px;" required>
						...
						mysqli_connect('localhost','root','','cde');
						mysqli_select_db($con,"cde");

					$d_program = mysqli_query($con,"SELECT * FROM user");
					while($getDprog = mysqli_fetch_array($d_program)){
						$id = $getDprog['UID'];
						$d_program1 = mysqli_query($con,"SELECT * FROM account where UID='$id'");
...
<tr><td>status</td><td><select name="status" id="status" style="height: 30px;width: 200px;"required>
