...
$sql="insert into user(UID,fname,lname,sex,Email,phone_No,location,photo,c_code) values('$uid','$fname','$lname','$sex','$email','$phone','$loc','$photopath','$co')";
							$inserted=mysqli_query($con,$sql)or die(mysqli_error($con));
							}
...
$sql="insert into user(UID,fname,lname,sex,Email,phone_No,location,photo,d_code,c_code) values('$uid','$fname','$lname','$sex','$email','$phone','$loc','$photopath','$dd','$co')";
							$inserted=mysqli_query($con,$sql)or die(mysql_error($con));
							}
					else {
...
session_start();
	...
	$status='yes';
	$da=date('y-m-d');
	mysqli_query($con,"INSERT INTO logfile (logid,username,role,status,start_time,activity_type,activity_performed,date,ip_address,end)  VALUES(' ','Admin','system admin','$status','$actual_time','add user',concat('uid[','$uid','] ','name[','$fname','] ','father name[','$lname','] ','sex[','$sex','] ','user id[','$uid','] ','phone[','$phone','] '),'$da','$ipaddress','')") or die (mysql_error());	
